...
function rumble_admin_grant_abilities(int $gameId, int $actorUserId, int $targetUserId, array $abilityIds): array
{
    ...
    $memberStmt = $db->prepare(
        'SELECT gm.role, u.username, u.is_active, rps.owned_abilities_json '
    ...
    if ((string)($member['role'] ?? '') === 'observer') {
        error_response('Observers cannot receive rumble abilities.', 409);
    }
    ...
    try {
        ...
        $auditStmt->execute([
            'game_id' => $gameId,
            'user_id' => $actorUserId,
            'action_type' => 'admin_grant_abilities',
            'payload' => json_encode([
