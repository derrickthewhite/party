...
export function createMafiaGameScreen(deps) {
	...
	function showIconPickerModal(currentIconKey, iconCatalog) {
		...
		availableIcons.forEach(function eachIcon(iconKey) {
			...
			// always add to the original folder group (if any)
			let folderKey = '';
			if (normalized) {
			...
			// also add to animals/humans group when applicable
			if (isPlayerIconAnimal(iconKey)) addToGroup('animals');
			if (isPlayerIconHuman(iconKey)) addToGroup('humans');
		});
